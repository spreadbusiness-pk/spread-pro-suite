...
export function computeTotals(input: Partial<CostBreakdown> & { override_selling?: number | null }): CostBreakdown {
  ...
  const die_cutting = num(input.die_cutting_cost);
  ...
  const total = paper + ctp + printing + die_cutting + ink + finishing + binding + labour + transport + misc;
